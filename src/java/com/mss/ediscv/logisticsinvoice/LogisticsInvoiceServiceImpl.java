...
public class LogisticsInvoiceServiceImpl implements LogisticsInvoiceService {
    ...
    public ArrayList<LogisticsInvoiceBean> buildLogInvoiceQuery(LogisticsInvoiceAction logisticsInvoiceAction, HttpServletRequest httpServletRequest) throws ServiceLocatorException {
        ...
        invSearchQuery.append("SELECT DISTINCT TOP 500("
                ...
                + "TP2.NAME as REC_NAME,TP1.NAME as SENDER_NAME,"
                ...
                + "FILES.STATUS as STATUS,FILES.DIRECTION as DIRECTION,"
...
public ArrayList<LogisticsInvoiceBean> buildLogInvoiceArchiveQuery(LogisticsInvoiceAction logisticsInvoiceAction, HttpServletRequest httpServletRequest) throws ServiceLocatorException {
        StringBuffer invSearchQuery = new StringBuffer();
        logger.info("Entered into the :::: LogisticsInvoiceServiceImpl :::: buildLogInvoiceQuery");
        ...
        invSearchQuery.append("SELECT DISTINCT TOP 500("
                + "ARCHIVE_FILES.FILE_ID) as FILE_ID,"
                + "ARCHIVE_TRANSPORT_INVOICE.INVOICE_NUMBER as INVOICE_NUMBER,ARCHIVE_TRANSPORT_INVOICE.ID as ID,ARCHIVE_FILES.ID AS IdFiles,"
                + "TP2.NAME as REC_NAME,TP1.NAME as SENDER_NAME,"
                + "ARCHIVE_TRANSPORT_INVOICE.PO_NUMBER as PO_NUMBER,"
                + "ARCHIVE_TRANSPORT_INVOICE.SHIPMENT_ID as SHIPMENT_ID,ARCHIVE_FILES.DATE_TIME_RECEIVED,"
                //TRANSPORT_INVOICE.SHIPMENT_ID
                + "ARCHIVE_TRANSPORT_INVOICE.TOTAL_WEIGHT as TOTAL_WEIGHT,ARCHIVE_FILES.ERR_MESSAGE,"
                + "ARCHIVE_TRANSPORT_INVOICE.TOTAL_AMOUNT as TOTAL_AMOUNT,"
                + "ARCHIVE_TRANSPORT_INVOICE.INVOICE_DATE as INVOICE_DATE,ARCHIVE_FILES.SENDER_ID,ARCHIVE_FILES.RECEIVER_ID,"
                + "ARCHIVE_FILES.STATUS as STATUS,ARCHIVE_FILES.DIRECTION as DIRECTION,"
                + "ARCHIVE_FILES.TMW_SENDERID as TMW_SENDERID,"
                + "ARCHIVE_FILES.TMW_RECEIVERID as TMW_RECEIVERID,"
                + "ARCHIVE_FILES.ACK_STATUS as ACK_STATUS"
                + " FROM ARCHIVE_TRANSPORT_INVOICE "
                // +"LEFT OUTER JOIN ARCHIV_FILES"
                // +" ON (ARCHIVE_TRANSPORT_INVOICE.INVOICE_NUMBER = ARCHIVE_FILES.PRI_KEY_VAL "
                //+" AND "
                // +"TRANSPORT_INVOICE.SHIPMENT_ID=ARCHIV_FILES.SEC_KEY_VAL)"
                + "LEFT OUTER JOIN ARCHIVE_FILES ON (ARCHIVE_TRANSPORT_INVOICE.FILE_ID =ARCHIVE_FILES.FILE_ID) "
                //                                            +" LEFT OUTER JOIN TP TP1 "
                //                                            +"ON (TP1.ID=ARCHIV_FILES.TMW_SENDERID) "
                //                                            +"LEFT OUTER JOIN TP TP2 "
                //                                            +"ON (TP2.ID=ARCHIV_FILES.TMW_RECEIVERID)");
        ...
        if (corrattribute != null && corrattribute.equalsIgnoreCase("Invoice Number")) {
            if (corrvalue != null && !"".equals(corrvalue.trim())) {
                invSearchQuery.append(WildCardSql.getWildCardSql1("ARCHIVE_TRANSPORT_INVOICE.INVOICE_NUMBER",
                        corrvalue.trim().toUpperCase()));
            }
        }
        if (corrattribute1 != null && corrattribute1.equalsIgnoreCase("Invoice Number")) {
        ...
        if (corrattribute != null && corrattribute.equalsIgnoreCase("Shipment Number")) {
            if (corrvalue != null && !"".equals(corrvalue.trim())) {
                invSearchQuery.append(WildCardSql.getWildCardSql1("ARCHIVE_TRANSPORT_INVOICE.SHIPMENT_ID",
                        corrvalue.trim().toUpperCase()));
            }
        }
        if (corrattribute1 != null && corrattribute1.equalsIgnoreCase("Shipment Number")) {
        ...
        if (corrattribute != null && corrattribute.equalsIgnoreCase("Instance ID")) {
            if (corrvalue != null && !"".equals(corrvalue.trim())) {
                invSearchQuery.append(WildCardSql.getWildCardSql1("ARCHIVE_FILES.FILE_ID",
                        corrvalue.trim().toUpperCase()));
            }
        }
        if (corrattribute1 != null && corrattribute1.equalsIgnoreCase("Instance ID")) {
        ...
        //DocType
        if (doctype != null && !"".equals(doctype.trim())) {
            invSearchQuery.append(WildCardSql.getWildCardSql1("ARCHIVE_FILES.TRANSACTION_TYPE",
                    doctype.trim()));
        }
        //Status
        if (status != null && !"-1".equals(status.trim())) {
            invSearchQuery.append(WildCardSql.getWildCardSql1("ARCHIVE_FILES.STATUS",
                    status.trim()));
        }
        //ACK_STATUS
        if (ackStatus != null && !"-1".equals(ackStatus.trim())) {
            invSearchQuery.append(WildCardSql.getWildCardSql1("ARCHIVE_FILES.ACK_STATUS",
                    ackStatus.trim()));
        }
        ...
        if (invSenderId != null && !"".equals(invSenderId.trim())) {
            invSearchQuery.append(" AND (ARCHIVE_FILES.SENDER_ID like '%" + invSenderId + "%' OR ARCHIVE_FILES.TMW_SENDERID like '%" + invSenderId + "%')");
        }
        ...
        if (invRecId != null && !"".equals(invRecId.trim())) {
            invSearchQuery.append(" AND (ARCHIVE_FILES.RECEIVER_ID like '%" + invRecId + "%' OR ARCHIVE_FILES.TMW_RECEIVERID like '%" + invRecId + "%')");
        }
        ...
        //  invSearchQuery.append(" order by ARCHIVE_FILES.DATE_TIME_RECEIVED DESC" );      
        invSearchQuery.append(" order by ARCHIVE_FILES.DATE_TIME_RECEIVED DESC");
        // documentSearchQuery.append(" WITH UR");
        System.out.println("Logistics Inv query-->" + invSearchQuery.toString());
        String searchQuery = invSearchQuery.toString();
